    ...
    return (
      <div className="p-8 max-w-5xl">
        <div className="animate-pulse space-y-4">
          ...
          {[1, 2].map(i => <div key={i} className="h-24 bg-white/40 rounded-2xl" />)}
        </div>
      </div>
    );
  }
      ...
      {list.length === 0 ? (
        ...
        <div className="grid gap-4">
          {list.map(p => (
            <div
              key={p.id as string}
              className="bg-white/60 backdrop-blur-xl rounded-2xl border border-white/30 shadow-sm p-5 hover:bg-white/70 transition-all duration-200"
            >
              <div className="flex items-start justify-between">
                <div className="flex items-start gap-3">
                  <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white text-sm font-bold shadow-sm shrink-0">
                ...
                <div className="flex gap-2 text-xs">
                  <span className="px-2.5 py-1 bg-white/60 border border-white/20 rounded-lg text-gray-600 font-medium">
