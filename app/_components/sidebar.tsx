    ...
    <aside className="w-56 min-h-dvh bg-white/50 backdrop-blur-xl border-r border-white/30 flex flex-col">
      <div className="p-5 border-b border-white/20">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-[35px] bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white text-sm font-bold shadow-sm">
            I
          </div>
          <div>
        ...
        {NAV.map(({ href, label, icon }) => {
          ...
          return (
            <Link
              key={href}
              href={href}
              className={`flex items-center gap-3 px-3.5 py-2.5 rounded-[35px] text-sm transition-all duration-200 ${
                active
